...
***** TODO explain a bit more for each of these? JPT lekhdeu. Buda harulai paragraph chaiyo re
...
There is only one external actor/ type of user. They can send data to the system by adding new cards or editing
existing ones. Card data can be retrieved from the system and shown to the user when editing or practicing cards.
#+CAPTION: Context Level diagram for CardsQL
#+attr_latex: :height 110px
#+attr_org: :height 110px
